...
namespace CommuntiyApiDemo.Controllers
{
    public class UserPostController : ApiController
    {
        ...
        public IHttpActionResult getFollowingUserPosts(int UID)
        {
            ...
            String query2 = "select FollowingID from FollowUser where UserID = '"+UID+"';";
            String query="";
            ...
            while (reader.Read())
            {
                followingIDS += "'" + reader[0].ToString() + "',";
            }
            reader.Close(); 
            followingIDS = followingIDS.Remove(followingIDS.Length - 1);
            /**/
            query = "select * from Userposts where userID IN(" + followingIDS + ");";
